...
/eval-check RHAISTRAT-1742 http://localhost:9184 --workspace=~/Desktop/rhoai-mr-184

# Same thing with auto-fix loop (up to 3 attempts)
/eval-check RHAISTRAT-1742 http://localhost:9184 --workspace=~/Desktop/rhoai-mr-184 --fix

# Explicit: run phase A only, no usability
eval-iterate RHAISTRAT-1742 http://localhost:9184 --phase-a-only --max-iterations=3

# Later, when ready for usability testing:
eval-iterate RHAISTRAT-1742 http://localhost:9184 --phase-b-only
# (Reuses Phase A artifacts — doesn't re-run AC validation)
```

## What the Designer Gets (Example Output)

After running `/eval-check` on RHAISTRAT-1742:

```
Eval check complete: RHAISTRAT-1742 — Deploy agent images
  ...
  • Re-run: /eval-check RHAISTRAT-1742 http://localhost:9184
